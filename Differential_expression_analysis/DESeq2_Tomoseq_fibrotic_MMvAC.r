...
output_dir="/home/a.kukkonen_cbs-niob.local/PartB/deseq2_output/tomoseq_fibrotic_MMvAC/"
...
count_data <- count_data %>% select(matches("_MI14D."))
...
# Subset the fibrotic slices for normalization (more slices than in DE step)
slice_numbers <- c(16:24)
subset_for_norm <- slice_subset(count_data = count_data, meta_data = meta_data, slice_numbers = slice_numbers)
counts_for_norm <- subset_for_norm[["count_data_subset"]]
meta_for_norm <- subset_for_norm[["meta_data_subset"]]
rm(subset_for_norm)

check_input_format(count_data = counts_for_norm, meta_data = meta_for_norm)

# Apply an expression filter for normalization step
counts_for_norm <- expression_filter(counts_for_norm, count_threshold = 2, sample_n_threshold = 30)
sample_size_factors <- estimate_size_factors(counts_for_norm, meta_for_norm)
...
saveRDS(counts_for_de, paste0(output_dir, "DESeq2_input_counts_unfilt_Tomoseq_fibrotic_MMvAC.RData"))
...
counts_for_de <- expression_filter(counts_for_de, count_threshold = 10, sample_n_threshold = 12)
...
png(file = paste0(output_dir, "DispEst_Tomoseq_fibrotic_MMvAC.png"), width = 1200, height = 900)
plotDispEsts(dds)
dev.off()
...
png(file = paste0(output_dir, "PCA_Tomoseq_fibrotic_MMvAC_RLog.png"), width = 600, height = 350)
...
write.table(res, paste0(output_dir, "DESeq2_output_Tomoseq_fibrotic_MMvAC.tsv"), sep="\t", col.names=T, quote=F, append=F, row.names=T)
